...
const QUESTIONS = [
  ...
  { key: "stress", label: "How stressed do you feel?", options: STRESS, Icon: Flower2 },
];
...
export default function WellnessCheckIn({ onAskAi }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        ...
        {plan ? (
          ...
          <div className="flex items-center gap-2">
            <span
              className="text-[10px] uppercase tracking-[0.18em] font-heading font-medium"
              style={{ color: PROGRESS_TEXT }}
            >
              Answer 3 Questions
            </span>
      ...
      {plan ? (
        <WellnessPlanResult plan={plan} onAskAi={onAskAi || (() => window.alert("AI assistant coming soon"))} />
      ) : (
        <>
          <div className="grid md:grid-cols-3 divide-x divide-[#ECEEEC] border-t border-[#ECEEEC] pt-8">
            {QUESTIONS.map(({ key, label, options, Icon }) => (
              <div key={key} className="px-6 first:pl-0 last:pr-0 flex flex-col items-center text-center">
                ...
                <p
                  className="font-heading font-bold text-[11px] uppercase tracking-[0.14em] mt-4 mb-5"
                  style={{ color: INK }}
                >
                  {label}
                </p>
